...
def removeBlueFrame(image_path):
    ...
    thresh = cv2.threshold(
        blur_gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]
    ...
    median = cv2.medianBlur(dilate, 21)

    # Fill holes in frame with vertical and horizontal opening
    kernel2 = cv2.getStructuringElement(cv2.MORPH_RECT, (500, 1))
    res = cv2.morphologyEx(median, cv2.MORPH_OPEN, kernel2)
...
def removeFrameUsingHoughLines(image_path, dilation_kernel=20):
    ...
    kernel = cv2.getStructuringElement(
        cv2.MORPH_RECT, (dilation_kernel, dilation_kernel))
    ...
    lines = cv2.HoughLinesP(image=edges, rho=1, theta=np.pi/180, threshold=100,
                            lines=np.array([]), minLineLength=10, maxLineGap=500)
    ...
    if lines is not None:
        ...
        for i in range(np.shape(lines)[0]):
            cv2.line(lines_out, (lines[i][0][0], lines[i][0][1]), (
                lines[i][0][2], lines[i][0][3]), (255, 255, 255), 10, cv2.LINE_AA)

        # Using Closing to fill
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (100, 100))
        closing = cv2.morphologyEx(
            lines_out, cv2.MORPH_CLOSE, kernel, iterations=1)
...
def removeFoldingRule(image_path):
    ...
    no_frame = removeFrameUsingHoughLines(
        image_path=image_path, dilation_kernel=20)
    ...
    # Try with smaller dilation kernel
        ...
        no_frame = removeFrameUsingHoughLines(
            image_path=image_path, dilation_kernel=10)
