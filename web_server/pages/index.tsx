      ...
      <div className="flex flex-col items-center justify-center">
        ...
        <div className="flex flex-col items-start mx-auto mb-10 text-xs ">
          ...
          <div className="flex items-start justify-center space-x-2">
            ...
            <p>
              Betyder att någon är inne och tittar på denna mask eller bild just nu,
              <br /> notera att denna sida är statisk, dvs för att se uppdateringar måste sidan uppdateras. <br />
              Notera även API:n inte hinner notera att du lämnat en bild innan denna sida läses in om du klickar bakåt,
              <br />
        ...
        {imagesData.length > 0 &&
          imagesData
            ...
            .map((path: Data, index: number) => (
              <div key={path.fileName} title={path.uploadDate} className="flex flex-row items-end w-full justify-between  min-w-[50%] border-b my-1">
                <a className="flex space-x-4 justify-center items-center text-center" href={`/${path.fileName}`}>
                  {" "}
                  {getStarredIcon(path.star)}
                  <div className="w-6 flex text-center justify-center items-center">
                    {path.isViewed && (
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                        <path
                          strokeLinecap="round"
...
export async function getServerSideProps() {
  ...
  let res = await axios.get(`http://${process.env.NEXT_PUBLIC_IMAGES_API_HOST ?? "localhost:8000"}/images`, {
  ...
  for (let i = 0; i < images.length; i++) {
    let image = images[i];
    // console.log({ image });
    let data = res.data["images"].find((item: any) => item.filename === image);
    // console.log({ data });
    let mask = masks.find((mask: string) => mask.includes(image.split(".")[0]));
    let inProgress = data["inProgress"] as boolean;
    let uploadDate = data["uploadDate"] as string;
    let star = data["star"] as boolean;
    let isViewed = (data["isViewed"] as boolean) ?? false;
    // console.log({ image, mask, inProgress, uploadDate, star, isViewed });
